...
const Modal:React.FC<Modalprops> = ({closeEditModal, todoId, oldTitle, oldDescription}) => {
   ...
   const dispatch = useAppDispatch()
    const handleSubmit = async (e:FormEvent<HTMLFormElement>)=>{
        ...
        try {
            ...
            await dispatch(editTodos({id:todoId, data: {
              title:todoTitle,description:todoDescription, attachment:null
            }}))
        } catch (error) {
            console.log(error)
            setLoading(false)
  ...
  <form onSubmit={handleSubmit}>
    ...
    <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
      <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
        ...
        <h1 className="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Enter Todo Details</h1>
        <label htmlFor="title" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Title</label>
        <input id="title" value={todoTitle} className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="James" />
        ...
        <div className="relative mb-5 mt-2">
          ...
          <input id="descriptiom" value={todoDescription} className="text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-16 text-sm border-gray-300 rounded border" placeholder="XXXX - XXXX - XXXX - XXXX" />
        </div>
        ...
        <div className="flex items-center justify-start w-full">
          ...
          <button className="focus:outline-none focus:ring-2 focus:ring-offset-2  focus:ring-gray-400 ml-3 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm" onClick={closeEditModal}>Cancel</button>
        </div>
        <button className="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" onClick={closeEditModal} aria-label="close modal" role="button">
  ...
  <div className="w-full flex justify-center py-12" id="button">
    <button className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 mx-auto transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-4 sm:px-8 py-2 text-xs sm:text-sm">Open Modal</button>
  </div>
  </form>
